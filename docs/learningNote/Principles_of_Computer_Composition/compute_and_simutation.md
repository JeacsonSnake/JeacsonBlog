...
### 1.二分算法 (Bisection Method)

**使用前提:** 与 *二分法* 相似

假设 `f(x)` 在区间 $[a, b]$ 上连续，且 $f(a)f(b) < 0$，即根在该区间内。则进行以下计算：

- 计算中点 m = (a + b)/2
- 判断 f(a)f(m) 的正负值：
  - 若小于零（根在区间$[a, m]$内）则使 b 移动至 m 位
  - 若大于于零（根不在区间$[a, m]$内）则使 a 移动至 m 位
- 计算真截断误差 Et 与绝对截断误差 Ea：
  - Et = abs(RealNumber - 现在的m)
  - Ea = abs(现在的m - 前一个m)
- 计算后的绝对截断误差小于给定误差值时循环结束

用伪代码可写为：

```txt
设置给定真正结果 RealNumber 值

while (Ea > 给定值，例如$10^{-10}$){
    计算中点  
    m = (a + b)/2
...
### 2.牛顿-拉弗森法（Newton-Raphson method）

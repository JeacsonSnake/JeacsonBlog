...
∴ 最大机器数值 = (-1)^2^ × (1.1...1)~2~ × 2^127^ ≈ **± 3.4 × 10^38^**

   最小机器数值 = (-1)^2^ × (1.0...0)~2~ × 2^-126^ ≈ **± 1.2 × 10^-38^**
...
∴ 最大机器数值 = (-1)^2^ × (1.1...1)~2~ × 2^1023^ ≈ **± 1.8 × 10^308^**

   最小机器数值 = (-1)^2^ × (1.0...0)~2~ × 2^-1022^ ≈ **± 2.2 × 10^-308^**
...
到此所有过程与[前文所述](./README.md#3十进制转机器数二进制)一致。但以下过程有所不同:
...
::: tip
这里的估计法与下文的[舍入规则](./README.md#9舍入规则rounding-rule)相似，可以相互参考
:::
...
::: tip
这里的估计法与上文的[估计法](./README.md#8-真误差true-error--真实相对误差-true-relative-error)相似，可以相互参考
:::

...
#include "base_interfaces_demo/action/progress.hpp"
#include <cstdlib> 
// 与 atoi 相关的库在印象中好像并未被引用就已被正常编译，原因未知，现已添加
...
    // 3-2. 发送请求；
    ...
    void result_callback(const rclcpp_action::ClientGoalHandle<Progress>::WrappedResult &result) {
        // 通过状态码 result.code 判断最终结果状态
        
        if (result.code == rclcpp_action::ResultCode::SUCCEED) {
...
在通过回调函数实现处理最终响应结果时，我们通过状态码 `result.code` 判断最终结果的状态。在这里的状态码一共有四个，存在于一个枚举类中。它们分别为:

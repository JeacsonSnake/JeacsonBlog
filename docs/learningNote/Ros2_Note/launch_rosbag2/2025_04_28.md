...
因为 `XML` & `YAML` 语言自身**并无事件触发函数**能进行适配，除非你需要**仅**使用 `C++` 进行相关代码的编写，否则基本不推荐使用 `XML`/`YAML` 语言作为编写 launch 的首要语言。若有实际需要，具体请参考[这篇有关XML/YAML的文章](2026_01_22.md)以及[官方文档](https://docs.ros.org/en/jazzy/Tutorials/Intermediate/Launch/Creating-Launch-Files.html)

---
title: ROS2-017-ROS2工具：启动文件 launch 的 python 实现
icon: 'kitchen-set'
date: 2026-01-21
category:
  - Computer
...
```bash
ros2 launch cpp01_launch py04_args_launch.py bg_r:=100 bg_g:=86
```

当然，如果不传入参数，则文件执行时会使用当时声明参数时所给定的默认值 `default_value` 进行相关解析。

::: info 动态传参所使用的参数名需要做区别
通过 `bash` 动态传入参数时需要注意的是：所传入参数名应与声明参数（变量）时 *`name` 自变量* 内所使用的参数名**相同**，而**并非**与声明参数（变量）时所使用的参数名 **本身** 相同。
:::
...
在这篇文章中，我们较为详尽的介绍了有关 “如何使用 `pyhton` 进行 `launch` 文件的编写” 的话题，除非你希望**仅**使用 `C++` 进行编写，否则由于 `XML` & `YAML` 语言自身**并无事件触发函数**能进行适配，并不推荐使用 `XML`/`YAML` 语言作为编写 launch 的首要语言。若有实际需要，请参考[下一篇文章](2026_01_22.md)以及[官方文档](https://docs.ros.org/en/jazzy/Tutorials/Intermediate/Launch/Creating-Launch-Files.html)。

...
## 二、使用 leancloud 做数据库，将博客用评论功能 waline 部署到 vercel 服务端时发生的问题

参考:[vuepress-plugin-comment2 插件中有关 配置waline的相关文档](https://plugin-comment2.vuejs.press/zh/guide/waline.html)

:::caution
2026-06-29 更新：上述网站已被修改，现在请参考[VuePress 生态系统中有关 配置 waline 的相关文档](https://ecosystem.vuejs.press/zh/plugins/blog/comment/waline/)。（内部相关配置由于 leancloud 不再支持，已实际进入半弃用状态）
:::
...
## 四、部署网站时，加载不出CSS/JS相关样式的问题

**前提**：在部署该博客后，又尝试在GITHUB Pages 上部署了另一个项目[博饼·状元筹](https://github.com/JeacsonSnake/bobing-jettonman-ALLinONE)。结果部署上去之后，发现加载不出CSS/JS相关样式，开启开发者工具后发现针对该类文件的请求全部404。
...
## 五、博客评论区专用数据库从 leancloud 迁移至 Neon PostgreSQL 相关问题
